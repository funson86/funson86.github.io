...
- init_server_components() 初始化内部组件 MDL_map::init()MDL锁（元数据锁）初始化  Query_cache::init()初始化查询缓存 randominit()随机数初始化 init_slave_list()如果主从复制初始化从库
...
mysql会对每一个连接分配一个线程thd，该thd记录所有的参数。
